...
import { new_config } from '../_utils/options'
import { setPassword } from 'keytar-sync'
...
export class Config {
        ...
        try {
            yargsInteractive()
                ...
                .then(async (configResults) => {
                    ...
                    // if (setMids === 'OK') {
                        displayLine('Configuration Saved', 'success')
                    // }
                });
        } catch (err) {
            console.log(err);
    ...
    //         config.apiClientSecret = await crypto.decrypt(
    ...
    //                 ).filter(
    //                     (item) =>
    //                         item.account !== 'io' && item.account !== 'salty'
    //                 );
    ...
    //                 let checkDeletion = await keytar.getPasswordSync(
    //                     'bldr',
    //                     instance
    //                 );
