...
const updateManifest = async (context: string, content: {
  assets?: object[],
  folders?: object[]
}) => {
  ...
  const manifestPath = rootPath
    ...
    if(!fileExists(manifestPath)) {
      ...
      if (state) {
        await createFile(manifestPath, JSON.stringify({
        ...
        for (const i in AssetTypeItems) {
          ...
          if (Object.prototype.hasOwnProperty.call(updateItem, 'id')) {
            ...
            if (Object.prototype.hasOwnProperty.call(updateItem, 'assetType')) {
              ...
              itemId = objectIdKey && Object.prototype.hasOwnProperty.call(updateItem, objectIdKey) && updateItem[objectIdKey];
            }
          }
          ...
          const manifestObj = manifestContextItems.find(
            ({ id }) => id === itemId
          );
          ...
          if (typeof manifestObj === 'undefined') {
            ...
            if (!isEqual(updateItem, manifestObj)) {
              const updateIndex = manifestContextItems.findIndex(
                ({ id }) => id === updateItem.id
              );
