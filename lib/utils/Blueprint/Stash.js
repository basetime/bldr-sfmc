...
module.exports = class Stash {
    ...
    async _setStashObj(dirPath, obj, newAsset) {
        ...
        if (newAsset) {
            switch (assetType) {
                case 'webpage':
                case 'htmlemail':
                    obj.views.html.content = `${await utils.scrubConfig(
                        file.toString()
                    )}`;
                    break;
                case 'codesnippetblock':
                    obj.content = `${await utils.scrubConfig(file.toString())}`;
                    break;
                case 'textonlyemail':
                    obj.views.text.content = `${await utils.scrubConfig(
                        file.toString()
                    )}`;
                    break;
                default:
                    file = await utils.scrubConfig(JSON.stringify(file));
                    obj = JSON.parse(file);
            }
            ...
            switch (assetType) {
                case 'string':
                    obj.fileContent = `${await utils.scrubConfig(
                        file.toString()
                    )}`;
                    break;
                default:
    ...
    async _getManifestAssetData(obj) {
        const dirPath = await this.localFile._getRootPath(contextMap);
        const manifestJSON = await this.localFile._getManifest(dirPath);
        return manifestJSON;
    }
