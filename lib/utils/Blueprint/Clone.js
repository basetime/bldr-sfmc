...
module.exports = class Clone {
  ...
  async cloneFromFolder(argv, context) {
    ...
    await this.localFile.manifestJSON(null, 'contentBuilder', { folders: folderPaths })
    await this.localFile.manifestJSON(null, 'contentBuilder', { assets: assetsPost })
  }
  ...
  async _formatDataForFile(results, folderPaths) {
    try {
      if (Array.isArray(results) && results.length !== 0) {
        const formatted = results.map((asset) => {
