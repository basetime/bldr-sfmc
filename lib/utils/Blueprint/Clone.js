...
module.exports = class Clone {
  constructor(bldr, localFile) {
    this.bldr = bldr;
    this.localFile = localFile;
  }

  async cloneFromFolder(argv) {
    const id = argv.f;
    ...
    await this.localFile.createEditableFiles(assetsPost);
    // Append or create .gitignore file
    await this.localFile.gitIgnore();
    // Update ManifestJSON file with responses
    await this.localFile.manifestJSON('contentBuilder', {folders: folderPaths})
    await this.localFile.manifestJSON('contentBuilder', {contentBlocks: assetsPost})
  }
