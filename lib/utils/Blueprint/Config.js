...
module.exports = class Config {
            ...
            yargsInteractive()
                ...
                .then(async (configResults) => {
                    ...
                    const configured = {
                        ...
                        apiClientId: await crypto.encrypt(
    ...
    async get(instance, show) {
        ...
        if (config && show) {
            ...
            config.apiClientId = await crypto
                .decrypt(config.apiClientId)
            config.apiClientSecret = await crypto
                .decrypt(config.apiClientSecret)
    ...
    async list(argv) {
        try {
            let config = typeof argv.l === 'string'
                ? await keytar.getPassword('bldr', argv.l)
                : await (await keytar.findCredentials('bldr')).filter(item => item.account !== 'io' && item.account !== 'salty');
    ...
    async remove(argv) {
        try {
            if (typeof argv.r !== 'string' && typeof argv.remove !== 'string')
                throw new Error(
                    `Please provide an instance name after the -r flag.`
                );
