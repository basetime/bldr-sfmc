...
module.exports = class Config {
    ...
    async init(argv) {
        ...
        try {
            const env = await crypto.envFile();
    ...
    async get(instance, show) {
        ...
        if (instance && show) {
            ...
            resp.apiClientId = await crypto
                .decrypt(resp.apiClientId)
                .substring(0, 5);
            resp.apiClientSecret = await crypto
                .decrypt(resp.apiClientSecret)
                .substring(0, 5);
    ...
    async list(argv) {
        try {
            const configurationResp = argv.l
                ? await this.get(argv.l)
                : await this.get();
