...
module.exports = class CBClone {
    ...
    async cloneFromFolder(argv, context) {
        try {
            const id = argv.f;
            let checkRootFolder = await this.bldr.folder.get(
                'asset',
                id,
                false
            );
            ...
            if (checkRootFolder.OverallStatus === 'OK') {
                ...
                pathToRoot = pathToRootArray
                    .splice(0, parentFolderIndex)
                    .join('/');
            }

            // Get all subfolder data staring with a root Id provided by the user
            const folderPaths = await this.getFoldersRecursiveDESC(
                id,
                pathToRoot
            );
            ...
            console.log(JSON.stringify(assetsPost, null, 2));
    ...
    async cloneFromID(id, obj = null, legacy = false) {
        try {
            ...
            const getFolderPaths = await this.setFolderPathRecursiveAsc(
                folderPaths
            );
