...
module.exports = class ASSearch {
    ...
    async asset(searchKey, searchTerm) {
        try {
            const resp = await this.bldr.automation.search(
                searchKey,
                searchTerm
            );

            if (!resp.OverallStatus === 'OK')
                throw new Error(resp.OverallStatus);
            ...
            const displayContent = resp.Results.map((result) => {
                return [
                    ...
                    new Column(
                        `${utils.automationStatus(result.Status)}`,
                        width.c1
                    ),
                    new Column(
     ...
     async activity(searchActivity, searchTerm) {
        try {
            ...
            const displayContent = resp.items.map((result) => {
                return [
                    new Column(`${result.name}`, 55),
                    new Column(`${result.queryDefinitionId}`, 40),
