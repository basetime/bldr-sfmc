...
module.exports = class Deploy {
    ...
    async deployContentBuilderAssets(contentBuilderAssets, contextDetails) {
        try {
            ...
            await noDependencyAssets.forEach((asset) => this.deployContentBuilderAsset(asset, contextDetails, false))
            await dependencyAssets.forEach((depAsset) => this.deployContentBuilderAsset(depAsset, contextDetails, true))
    ...
    async deployContentBuilderAsset(contentBuilderAsset, contextDetails, dependencies = false) {
        ...
        if (!dependencies) {
            ...
            const createdDependencyAssets = assetDependencies.map(
                dependencyBldrId => manifestJSON[contextDetails.context]['assets'].find((manifestJSONCBObj) => manifestJSONCBObj.bldrId === dependencyBldrId)
            );

            console.log(createdDependencyAssets)
