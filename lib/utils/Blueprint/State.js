...
module.exports = class State {
    ...
    get(key, show) {
        try {
            if (!key) {
                if (show)
                    console.log(
                        utils.assignObject(this.stateConfiguration.get())
                    );

                return utils.assignObject(this.stateConfiguration.get());
            } else {
                if (key && this.stateConfiguration.has(key))
                    if (show) console.log(this.stateConfiguration.get(key));
    ...
    set(argv) {
        try {
            ...
            if (!clientConfig.mids.find((bu) => bu.mid === midToSet))
    ...
    update(update) {
        if (!update.key) throw new Error('Key required');

        if (!update.value) throw new Error('Value required');
