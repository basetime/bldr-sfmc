            ...
            .then(async (optionsResult) => {
                try {
                    ...
                    const displayContent = [
                        [new Column(`${err.message}`, width.c3)]
                    ]
            ...
            for (const p in pathArr) {
                ...
                if (typeof manifestFolderObj === 'undefined') {
                    if (typeof parentId === 'undefined') {
                        const parentObj = await this.bldr.folder.search('asset', 'Name', rootFolder)

                        if (parentObj.OverallStatus !== 'OK') {
                            throw new Error(parentObj.OverallStatus)
                        }

                        if (!Object.prototype.hasOwnProperty.call(parentObj, 'Results')) {
