    ...
    async getByFolderArray(folderIds, cb_clone) {
        try {
            ...
            const automationStepDefinitions =
                await this.getAutomationActivities(automations, cb_clone);
                ...
                if (Object.prototype.hasOwnProperty.call(automation, 'steps')) {
                    for (const as in automation.steps) {
                        ...
                        for (const sa in steps.activities) {
                            ...
                            const assetType = await utils.identifyAutomationStudioActivityType(
                            ...
                            if (assetType) {
                                ...
                                if (assetType.name === 'userinitiatedsend') {
                                    const soapResp = await this.soap.retrieve('EmailSendDefinition', emailSendDefinition, {
                                        filter: {
                                    ...
                                    if (assetType.name === 'filteractivity') {
                                        ...
                                        const filterDefinition = await this.rest.get(`/automation/v1/filterdefinitions/${activityDefinitionId}`)
