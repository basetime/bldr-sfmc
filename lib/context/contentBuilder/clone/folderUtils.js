...
const getFolderPath = async (nextParentId, currentFolderName, path, client) => {
  do {
    const folderObj = await getFolder(nextParentId, client)

    if(folderObj.hasOwnProperty('parentId')){
      currentFolderName = folderObj.name;
      nextParentId = folderObj.parentId;
...
const compileSFMCFoldersASC = async (rootFolderName, parentId, client) => {
  let currentFolderName;
  let nextParentId = parentId;
  let path = rootFolderName;
  
  let folderPath = await getFolderPath(nextParentId, currentFolderName, path, client)
  return folderPath
}



const compileSFMCFoldersDESC = async (id, client) => {
  try {
...
module.exports.cloneFoldersDESC = async (argv, client) => {
  try {
    console.log('getting sfmc folders');
    const folderPaths = await compileSFMCFoldersDESC(argv.f, client);
...
module.exports.cloneFoldersASC = async (rootFolderName, parentId, client) => {
  try {
    console.log('getting sfmc folders');
    const folderPath = await compileSFMCFoldersASC(rootFolderName, parentId, client);
    const folderPathArr = new Array(folderPath)

    console.log('cloning local folders');
    await createLocalDirs(folderPathArr);
