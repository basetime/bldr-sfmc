const { getParentFolderFromArray, localFileExists, createDir } = require('../../../utils/index')
...
const setFolderPaths = async (foldersArray, client) => {
  ...
  console.log(foldersOut)
  return foldersOut
}



const getFoldersRecursiveDESC = async (rootID, client) => {
  try {
...
const getFoldersRecursiveASC = async (folderObj, client) => {
  let path = folderObj.name;
  let parentId = folderObj.parentId;

  do {
    const folderResp = await getFolder(parentId, client)
    console.log(folderResp)
    parentId = folderResp.parentId;
    path = `${folderResp.name}/${path}`
  } while (parentId !== 0)
...
module.exports.cloneFoldersDESC = async (argv, client) => {
  try {
    ...
    const subFolders = await getFoldersRecursiveDESC(rootFolder.id, client)
...
module.exports.cloneFoldersASC = async (folderObj, client) => {
  try {
    return getFoldersRecursiveASC(folderObj, client)
  } catch (err) {
    console.log(err)
  }
}