const { localCreateFile, localFileExists } = require('../../../utils/index')
const { cloneFoldersASC } = require('./folderUtils')

const getAssetsByFolders = async (folderPaths, client) => {
  try {
...
module.exports.cloneAssetsFromFolder = async (folderPaths, client) => {
  ...
  const assets = await getAssetsByFolders(folderPaths, client)
  //console.log(assets)
...
module.exports.cloneAssetFromId = async (id, client) => {
  ...
  const folderObj = [{
    id: assets[0].category.id,
    name: assets[0].category.name,
    parentId: assets[0].category.parentId,
    categoryType: 'asset'
  }]

  const folderPath = await cloneFoldersASC(folderObj[0], client)
  assets[0].category.path = folderPath;
  folderObj[0].folderPath = folderPath;
